...
const LoginPage = () => {
  ...
  // function for login control
  const handleLogin = async () => {
    try {
      // Make a POST request to the login API
      const response = await axios.post(
        'http://ec2-65-0-75-157.ap-south-1.compute.amazonaws.com:3000/api/v1/login',
        {
      ...
      if (!voidcookie) {
        ...
        if (session_token) {
          ...
          cookies.set('session_token', session_token, { path: '/', sameSite: 'strict', secure: true })
        }
      }
      router.push('/')
    } catch (error) {
