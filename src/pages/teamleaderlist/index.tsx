...
const VendorList: React.FC = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      const api = `http://ec2-65-0-75-157.ap-south-1.compute.amazonaws.com:3000/api/v1/team-leaders`
          ...
          {users.length > 0 ? (
            users.map(user => (
              <tr key={user.id} style={tableRowStyle}>
                ...
                <td style={tableCellStyle}>{user.teamLeader?.userId}</td>
                <td style={tableCellStyle}>{user.teamLeader?.adminId}</td>
                <td style={tableCellStyle}>{user.teamLeader?.createdAt}</td>
                <td style={tableCellStyle}>{user.teamLeader?.updatedAt}</td>
              </tr>
            ))
          ) : (
